          ...
          <div className="space-y-16">
            {experiences.map((exp, idx) => (
              <div key={idx} className={`relative flex flex-col md:flex-row ${idx % 2 === 0 ? 'md:flex-row-reverse' : ''} gap-8 items-start`}>
                ...
                {/* Timeline Dot */}
                <div className="absolute left-8 md:left-1/2 -translate-x-1/2 mt-1.5 z-10 flex items-center justify-center w-8 h-8 rounded-full bg-[#0f172a] border border-[#1d4ed8]">
                        ...
                        <div className="flex flex-wrap gap-3">
                          {exp.proofs.map((proof, pIdx) => (
                            <motion.a
                              ...
                              whileTap={{ scale: 0.95 }}
                              className="flex items-center gap-2 px-4 py-2 bg-[#0f172a] hover:bg-[#e2e8f0] border border-[#e2e8f0] rounded-lg text-sm text-[#334155] font-medium transition-colors"
                            >
